...
class Tabel():
    def __init__(self):
        ...
        self.step_name = 'abcdefghijklmnop'
        self.output_state = 'NNNNNNNNNNNNNNNN'
        self.step_save = set([x for x in self.step_name])
        self.step_dict = {}
        for i in range(16):
            self.step_dict[self.step_name[i]] = (i // 4, i % 4)
        self.if_end = False
        self.winner = 0
    ...
    def judge(self):
        ...
        result.extend([sum(self.state[[0, 1, 2, 3], [0, 1, 2, 3]]), sum(self.state[[0, 1, 2, 3], [3, 2, 1, 0]])])
        if np.max(result) == 4:
            self.if_end = True
            self.winner = 1
        elif np.min(result) == -4:
            self.if_end = True
            self.winner = -1
